...
public class RtcpPktRR extends RtcpPkt {
	...
	public int[] lossFraction = null;//-1; //8 bits
	/** Number of lost packets */
	public int[] lostPktCount = null;//-1; //24 bits
	/** Extended highest sequence received */
	public long[] extHighSeqRecv = null;//-1; //32 bits
	/** Interarrival jitter*/
	public long[] interArvJitter = null;//-1; //32 bits
	/** Middle 32 bits of NTP when last SR was received */
	public long[] timeStampLSR = null;//-1; //32 bits
	/** Delay on last SRC */
	public long[] delaySR = null;//-1; //32 bits
	 ...
	 */
	public RtcpPktRR(byte[] aRawPkt, int start, int rrCount) {
    ...
    public String toString() {
        ...
        buf.append("{RtcpPktRR， ssrc=").append(ssrc);
        if(null != reportees) {
            ...
            buf.append("]");
        } else if (null != reporteeSsrc){
            buf.append(", reporteeSsrc:[");
            for(int i=0;i<reporteeSsrc.length; i++) {
                buf.append("{");
                buf.append("ssrc=").append(reporteeSsrc[i]);
                buf.append(", timeStampLSR=").append(timeStampLSR[i]);
                buf.append("}");
            }
            buf.append("]");
